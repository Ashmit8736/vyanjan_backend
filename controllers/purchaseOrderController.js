import connectDB from "../config/db.js";
import {
  createPurchaseOrder,
  getPurchaseOrdersByBranch
} from "../models/purchaseOrderModel.js";

export const createPurchaseOrderController = async (req, res) => {
  try {
    const conn = await connectDB();
    const branch_id = req.user.branch_id;

    const {
      supplier_id,
      purchase_date,
      po_number,          // ✅ MANUAL PO
      invoice_number,
      tax_amount = 0,
      discount_amount = 0,
      payment_status = "pending",
      items
    } = req.body;

    if (!supplier_id || !purchase_date || !po_number || !items?.length) {
      return res.status(400).json({
        success: false,
        message: "Supplier, purchase date, PO number and items are required"
      });
    }

    // 🔒 CHECK DUPLICATE PO NUMBER (PER BRANCH)
    const [[existingPO]] = await conn.query(
      `SELECT id FROM purchase_orders
       WHERE po_number = ? AND branch_id = ?`,
      [po_number, branch_id]
    );

    if (existingPO) {
      return res.status(400).json({
        success: false,
        message: "PO number already exists"
      });
    }

    let subTotal = 0;
    const calculatedItems = [];

    for (const item of items) {
      const [[raw]] = await conn.query(
        `SELECT purchase_unit_id
         FROM raw_materials
         WHERE id = ? AND branch_id = ? AND is_active = 1`,
        [item.raw_material_id, branch_id]
      );

      if (!raw) {
        return res.status(400).json({
          success: false,
          message: `Invalid raw material ID ${item.raw_material_id}`
        });
      }

      const total = item.quantity * item.unit_price;
      subTotal += total;

      calculatedItems.push({
        raw_material_id: item.raw_material_id,
        purchase_unit_id: raw.purchase_unit_id,
        quantity: item.quantity,
        unit_price: item.unit_price,
        total_amount: total
      });
    }

    const grandTotal = subTotal + tax_amount - discount_amount;

    const data = [
      branch_id,
      supplier_id,
      po_number,               // ✅ MANUAL
      invoice_number || null,
      purchase_date,
      subTotal,
      tax_amount,
      discount_amount,
      grandTotal,
      payment_status
    ];

    const purchaseOrderId =
      await createPurchaseOrder(data, calculatedItems);

    res.status(201).json({
      success: true,
      message: "Purchase bill created (manual PO number, stock not updated)",
      purchase_order_id: purchaseOrderId
    });

  } catch (error) {
    res.status(500).json({
      success: false,
      message: error.message
    });
  }
};

export const getPurchaseOrdersController = async (req, res) => {
  try {
    const branchId = req.user.branch_id;

    const data = await getPurchaseOrdersByBranch(branchId);

    res.json({
      success: true,
      data
    });

  } catch (error) {
    res.status(500).json({
      success: false,
      message: error.message
    });
  }
};
